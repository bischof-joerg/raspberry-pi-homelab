...
- Which roadmap stage — R4 core (Traefik) or R5 apps (Stirling PDF, AdGuard Home, Home Assistant)?
...
R4 Traefik with a LAN-only Pi needs DNS-01 and a public domain; the provider must be supported by
Traefik's ACME DNS providers, and that is unverified. R5.2 AdGuard needs port 53 and a decision on
client DNS. R5.3 Home Assistant commonly wants host networking, which conflicts with the
